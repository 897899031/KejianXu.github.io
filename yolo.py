...
class YOLO(object):
    _defaults = {
        #--------------------------------------------------------------------------#
        #   Be sure to modify model_path and classes_path for predictions using your own trained model!
        #   model_path points to the weights file under the logs folder, and classes_path points to the txt under model_data
        #   There are multiple weights files in the logs folder after training, just choose the one with lower validation set loss.
        #   Lower loss in the validation set does not mean higher mAP, it only means that the weights have better generalization performance on the validation set.
        ...
        "input_shape" : [640, 640],
        #---------------------------------------------------------------------#
        #   Versions of YoloAX used: nano, tiny, s, m, l, x
        #---------------------------------------------------------------------#
        "phi" : 's',
        #---------------------------------------------------------------------#
        #   Only prediction boxes with scores greater than the confidence level are retained
        #---------------------------------------------------------------------#
        "confidence" : 0.5,
        #---------------------------------------------------------------------#
        #   Size of nms_iou used for non-extremely suppressed
        #---------------------------------------------------------------------#
        "nms_iou" : 0.3,
        #---------------------------------------------------------------------#
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        #---------------------------------------------------------#
        #   Convert the image to an RGB image here to prevent the grayscale map from reporting errors in the prediction.
        ...
        with torch.no_grad():
            ...
            if self.cuda:
                images = images.cuda()
            #---------------------------------------------------------#
            #   The images are fed into the network for prediction!
            #---------------------------------------------------------#
            outputs = self.net(images)
            outputs = decode_outputs(outputs, self.input_shape)
            #---------------------------------------------------------#
            #   Stacking of prediction frames followed by non-great suppression
            #---------------------------------------------------------#
        ...
        #---------------------------------------------------------#
        #   Whether or not to perform target tailoring
        #---------------------------------------------------------#
        if crop:
