...
if __name__ == "__main__":
    '''
    Recall and Precision are not an area concept like AP, so the Recall and Precision values of the network are different when the threshold value (Confidence) is different.
    By default, the Recall and Precision calculated in this code represent the corresponding Recall and Precision values when the threshold value (Confidence) is 0.5.

    Due to the limitation of the principle of mAP calculation, the network needs to obtain nearly all the prediction frames when calculating the mAP, so that it can calculate the Recall and Precision values under different threshold conditions.
    Therefore, the number of frames of the txt inside map_out/detection-results/ obtained by this code is generally more than that of the direct prediction, with the purpose of listing all possible prediction frames, the
    '''
    #------------------------------------------------------------------------------------------------------------------#
    ...
    image_ids = open(os.path.join(VOCdevkit_path, "VOC2012/ImageSets/Main/train.txt")).read().strip().split()
