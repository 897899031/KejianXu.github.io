...
class IOUloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # Coordinates of the upper left corner of the predicted and real boxes, take the maximum value
        tl = torch.max(
            (pred[:, :2] - pred[:, 2:] / 2), (target[:, :2] - target[:, 2:] / 2)
        )
        # Coordinates of the lower right corner of the predicted and real boxes, minimized
        br = torch.min(
            (pred[:, :2] + pred[:, 2:] / 2), (target[:, :2] + target[:, 2:] / 2)
        )
        # Width and height of predicted and real boxes
        w_pred = pred[:, 2] - pred[:, 0] / 2
        h_pred = pred[:, 3] - pred[:, 1] / 2
        w_target = target[:, 2] - target[:, 0] / 2
        h_target = target[:, 3] - target[:, 1] / 2

        area_p = torch.prod(pred[:, 2:], 1)  # projected frame area
        area_g = torch.prod(target[:, 2:], 1)  # Real frame area

        en = (tl < br).type(tl.type()).prod(dim=1)
        area_i = torch.prod(br - tl, 1) * en  # Intersection area
        area_u = area_p + area_g - area_i  # Area of concatenation
        iou = (area_i) / (area_u + 1e-16)  # Cross-over ratio
        # Coordinates of the upper-left corner of the prediction box and the real box, minimized
        c_tl = torch.min(
            (pred[:, :2] - pred[:, 2:] / 2), (target[:, :2] - target[:, 2:] / 2)
        )
        # Coordinates of the lower right corner of the predicted and real boxes, take the maximum value
        c_br = torch.max(
            (pred[:, :2] + pred[:, 2:] / 2), (target[:, :2] + target[:, 2:] / 2)
        )
        # Coordinates of the centers of the predicted and real boxes
        center_pred = (pred[:, :2] + pred[:, 2:]) / 2
        center_target = (target[:, :2] + target[:, 2:]) / 2

        # Square of the Euclidean distance between the centers of the prediction frame and the real frame
        dist = torch.sum(torch.pow(center_pred - center_target, 2), dim=1)
        # Square of the diagonal distance of the smallest outer rectangle
        d_area_c = torch.sum(torch.pow(c_br - c_tl, 2), dim=1)

        if self.loss_type == "iou":
            loss = 1 - iou ** 2
        elif self.loss_type == "giou":
            # Area of the smallest external rectangle
            area_c = torch.prod(c_br - c_tl, 1)
            giou = iou - (area_c - area_u) / area_c.clamp(1e-16)
            loss = 1 - giou.clamp(min=-1.0, max=1.0)
        elif self.loss_type == "diou":
            diou = iou - dist / d_area_c.clamp(1e-16)
            loss = 1 - diou.clamp(min=-1.0, max=1.0)
        elif self.loss_type == "ciou":
            # v denotes the parameter that measures the consistency of the aspect ratio
            v = (4 / math.pi ** 2) * (torch.atan(w_target / h_target) - torch.atan(w_pred / h_pred)).pow(2)
            with torch.no_grad():
                alpha = v / (1 - iou + v + 1e-16)
            ciou = iou - dist / d_area_c.clamp(1e-16) - alpha * v
            loss = 1 - ciou.clamp(min=-1.0, max=1.0)
        elif self.loss_type == "eiou":
            # Width and height of the smallest external rectangle
            w_area_c = torch.abs(pred[:, 2] - target[:, 0]) if pred[:, 0] > target[:, 0] else torch.abs(target[:, 2] - pred[:, 0])
            h_area_c = torch.abs(pred[:, 3] - target[:, 1]) if pred[:, 1] > target[:, 1] else torch.abs(target[:, 3] - pred[:, 1])
            dist_w = torch.sum(torch.pow(w_pred - w_target, 2), dim=1)
            dist_h = torch.sum(torch.pow(h_pred - h_target, 2), dim=1)
            eiou = iou - dist / d_area_c.clamp(1e-16) - dist_w / (w_area_c ** 2).clamp(1e-16) - dist_h / (h_area_c ** 2).clamp(1e-16)
            loss = 1 - eiou.clamp(min=-1.0, max=1.0)
        elif self.loss_type == "geiou":
            # Width and height of the smallest external rectangle
            w_area_c = torch.abs(pred[:, 2] - target[:, 0]) if pred[:, 0] > target[:, 0] else torch.abs(
                target[:, 2] - pred[:, 0])
            h_area_c = torch.abs(pred[:, 3] - target[:, 1]) if pred[:, 1] > target[:, 1] else torch.abs(
                target[:, 3] - pred[:, 1])
            dist_w = torch.sum(torch.pow(w_pred - w_target, 2), dim=1)
            dist_h = torch.sum(torch.pow(h_pred - h_target, 2), dim=1)
            eiou = iou - dist / d_area_c.clamp(1e-16) - dist_w / (w_area_c ** 2).clamp(1e-16) - dist_h / (
                        h_area_c ** 2).clamp(1e-16)
            loss_eiou = 1 - eiou.clamp(min=-1.0, max=1.0)
            # Generalized Focal Loss
            logit = 1
            logit = torch.log(logit)
            logit = torch.where(torch.eq(target, 1), logit, -logit)
            logit = torch.where(torch.ne(target, 1), logit, torch.zeros_like(logit))
            logit = torch.where(torch.ne(target, 0), logit, torch.zeros_like(logit))
            logit = torch.where(torch.eq(target, 0), logit, -torch.ones_like(logit))

            logit = torch.where(torch.eq(target, 1), logit, logit + 1000)
            logit = torch.where(torch.eq(target, 0), logit, logit - 1000)

            prob = torch.exp(logit)
            prob = torch.where(torch.eq(target, 1), prob, 1 - prob)
            prob = torch.where(torch.ne(target, 1), prob, torch.zeros_like(prob))
            prob = torch.where(torch.ne(target, 0), prob, torch.zeros_like(prob))
            prob = torch.where(torch.eq(target, 0), prob, 1 - prob)
            ...
            if self.beta is None:
                beta = torch.ones_like(prob)
            else:
                beta = self.beta

            alpha = alpha * (1 - prob) ** self.gamma
            loss_gfl = -torch.sum(alpha * logit, dim=1) / torch.sum(alpha, dim=1)
            loss = loss_gfl ** beta * loss_eiou
...
class YOLOLoss(nn.Module):
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        #   Then sum and determine how many points should be used for that box prediction
        ...
        for gt_idx in range(num_gt):
            # ------------------------------------------------------------#
            #   Select the smallest dynamic k points for each real frame
            # ------------------------------------------------------------#
        ...
        if (anchor_matching_gt > 1).sum() > 0:
            # ------------------------------------------------------------#
            #   When a feature point points to more than one real frame
            #   Pick the real frame with the smallest cost.
            # ------------------------------------------------------------#
        ...
        #   num_fg is the number of feature points for positive samples
        # ------------------------------------------------------------#
        fg_mask_inboxes = matching_matrix.sum(0) > 0.0
        num_fg = fg_mask_inboxes.sum().item()

        # ------------------------------------------------------------#
        #   Make updates to fg_mask
        # ------------------------------------------------------------#
        fg_mask[fg_mask.clone()] = fg_mask_inboxes

        # ------------------------------------------------------------#
        #   Obtain the type of item corresponding to the feature point
        # ------------------------------------------------------------#
